/** How long the bar stays visible after a removal or a failed undo, in ms. */
 ...
 * (docs/plan.md, F20): hidden, shown (hidden at hideAt), undoing (the mark
 * runs again, no time limit) or failed (the undo failed, hidden at
 * hideAt). Times are in ms, as from Date.now().
 */
export type UndoBarState =
  | { status: "hidden" }
  | { status: "shown"; item: RemovedItem; hideAt: number }
  | { status: "undoing"; item: RemovedItem }
  | { status: "failed"; item: RemovedItem; hideAt: number };
...
export type UndoBarAction =
  ...
  | { type: "undoFailed"; productId: string; now: number };
 ...
 *   - tick: hides a shown or failed bar whose time is up.
 ...
 *     for undoBarDuration and offers [Rückgängig] again.
 *
...
export function undoBarReducer(state: UndoBarState, action: UndoBarAction): UndoBarState {
  switch (action.type) {
    ...
    case "tick":
      if ((state.status === "shown" || state.status === "failed") && action.now >= state.hideAt) {
    ...
    case "undoFailed":
      ...
      return { status: "failed", item: state.item, hideAt: action.now + undoBarDuration };
  }
}
